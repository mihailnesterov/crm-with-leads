...
const getRowId = (row: IContacts) => row._id;
...
const DxContactsList: FC = () => {

    const [contacts] = useState(() => {
        const _contacts: any[] = [];
        for(let i=0; i < clients.length; i++) {
            const _client = clients[i];
            if( _client.contacts && _client.contacts.length > 0 ) {
                _client.contacts.map(c => {
                    _contacts.push(c);
                    c.client = _client.name;
                    c.manager = _client.manager;
                });
            }
        }
        return _contacts;
    ...
    const [arrayToStringColumns] = useState<string[]>(['phones']);

    const [tableColumnExtensions] = useState([
        { columnName: 'name', wordWrapEnabled: true },
        { columnName: 'client', wordWrapEnabled: true },
        { columnName: 'phones', wordWrapEnabled: true },
...
export default React.memo(DxContactsList);
