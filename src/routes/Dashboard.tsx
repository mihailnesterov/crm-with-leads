...
const Dashboard: FC = () => {
    ...
    const [regions] = useState(() => {
        ...
        for(let i=0; i < clients.length; i++) {
            const _client = clients[i];
            if( _client.region && _client.region.length > 0 ) {
                _client.region.forEach(c => {
                    _regions.push({"region":c,"client":_client.name});
                });
            }
        }
    ...
    return (
        <div>
            <header className={classes.header}>
                <h2 className={classes.title}>Сводка</h2>
                <div>
                    <Button 
                        component={ Link }
                        to="/clients/add"
